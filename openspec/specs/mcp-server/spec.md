...
### Requirement: 单账号配置加载
系统 SHALL 通过 `.env` 文件或进程环境变量读取唯一的账号凭据（`WECHAT_APP_ID`、`WECHAT_APP_SECRET`），MUST NOT 支持或要求在工具调用时传入凭据。
...
工具失败时 SHALL 返回包含错误原因与处置提示的中文错误信息；任何错误信息、日志或返回值中 MUST NOT 出现 access_token 或 AppSecret。
